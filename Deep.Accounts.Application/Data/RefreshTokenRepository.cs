...
internal sealed class RefreshTokenRepository(AccountsDbContext db) : IRefreshTokenRepository
{
    ...
    public async Task<IReadOnlyCollection<RefreshToken>> GetActiveByAccountIdAsync(
        Guid accountId,
        CancellationToken cancellationToken = default
    ) =>
        await db
            .RefreshTokens.Where(rt =>
                ...
                && rt.ExpiryDateUtc > DateTime.UtcNow
            )
            .ToListAsync(cancellationToken);

    public void Insert(RefreshToken refreshToken) => db.RefreshTokens.Add(refreshToken);
