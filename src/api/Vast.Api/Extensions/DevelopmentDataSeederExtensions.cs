...
internal static class DevelopmentDataSeederExtensions
{
    public static async Task SeedDevelopmentDataAsync(this IApplicationBuilder app)
    {
        ...
        SeedUser[] seedUsers =
        [
            new("Admin", "User", "admin@Vast.local", [RoleNames.ItAdmin]),
            new("Manager", "User", "manager@Vast.local", [RoleNames.Manager]),
            new("Program", "Owner", "owner@Vast.local", [RoleNames.ProgramOwner]),
            new("Brand", "Ambassador", "ba@Vast.local", [RoleNames.BrandAmbassador]),
            new("Coordinator", "One", "coordinator1@Vast.local", [RoleNames.Coordinator]),
            new("Coordinator", "Two", "coordinator2@Vast.local", [RoleNames.Coordinator]),
            new("Coordinator", "Three", "coordinator3@Vast.local", [RoleNames.Coordinator]),
        ];
    ...
    {
        dbContext.Attach(Vast.Accounts.Domain.Accounts.Role.ItAdmin).State = EntityState.Unchanged;
        dbContext.Attach(Vast.Accounts.Domain.Accounts.Role.Manager).State = EntityState.Unchanged;
        dbContext.Attach(Vast.Accounts.Domain.Accounts.Role.ProgramOwner).State =
            EntityState.Unchanged;
        dbContext.Attach(Vast.Accounts.Domain.Accounts.Role.Coordinator).State =
            EntityState.Unchanged;
        dbContext.Attach(Vast.Accounts.Domain.Accounts.Role.BrandAmbassador).State =
            EntityState.Unchanged;
    }

    private static void AttachKnownProgramRoles(ProgramsDbContext dbContext)
    {
        dbContext.Attach(Vast.Programs.Domain.Users.Role.ItAdmin).State = EntityState.Unchanged;
        dbContext.Attach(Vast.Programs.Domain.Users.Role.Manager).State = EntityState.Unchanged;
        dbContext.Attach(Vast.Programs.Domain.Users.Role.ProgramOwner).State =
            EntityState.Unchanged;
        dbContext.Attach(Vast.Programs.Domain.Users.Role.Coordinator).State = EntityState.Unchanged;
        dbContext.Attach(Vast.Programs.Domain.Users.Role.BrandAmbassador).State =
            EntityState.Unchanged;
    }
